DebtList()
{
	...
	web_custom_request("GetHashValue",
	        ...
	        "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ws=\"http://ws.hashgen.bellintegrator.ru/\">"
   			...
   			"<soapenv:Body>"
      		"	<ws:getHashByConcat>"
         	"		<concat>{ctn}{pendingCharges}</concat>"
      		"	</ws:getHashByConcat>"
   			"</soapenv:Body>"
			"</soapenv:Envelope>",
	        LAST); 
	...
	lr_start_transaction("api_1_0_info_debtList");
		
	web_url("api_1_0_info_debtList",
		"URL=http://{endpoint}/api/1.0/info/debtList?hash={hash_qouted}&ctn={ctn}&pendingCharges={pendingCharges}",
		"Resource=1",
	...
	checkResult("api_1_0_info_debtList", lr_eval_string("{response}"), lr_eval_string("{token}"), lr_eval_string("{ctnban_ctn}"), "");
