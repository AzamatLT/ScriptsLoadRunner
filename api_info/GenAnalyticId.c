GenAnalyticId()
{
	...
	lr_start_transaction("api_1_0_info_genAnalyticId");
		
	web_url("api_1_0_info_genAnalyticId",
		"URL=http://{endpoint}/api/1.0/info/genAnalyticId?ctn={ctn}&hash={hash_qouted}&type={type}",
		"Resource=1",
	...
	checkResult("api_1_0_info_genAnalyticId", lr_eval_string("{response}"), lr_eval_string("{token}"), lr_eval_string("{ctn}"), "");
