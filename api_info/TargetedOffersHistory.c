TargetedOffersHistory()
{
	...
	web_custom_request("GetHashValue",
	        ...
	        "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:ws=\"http://ws.hashgen.bellintegrator.ru/\">"
   			...
   			"<soapenv:Body>"
      		"	<ws:getHashByConcat>"
         	"		<concat>{ctn}{login_mobile}</concat>" //ctn, channelType, response, direction, category, startDate, endDate, socType, login
      		"	</ws:getHashByConcat>"
   			"</soapenv:Body>"
			"</soapenv:Envelope>",
	        LAST);
	...
	lr_start_transaction("api_1_0_info_targetedOffersHistory");
		
	web_url("api_1_0_info_targetedOffersHistory",
		"URL=http://{endpoint}/api/1.0/info/targetedOffersHistory?hash={hash_qouted}&ctn={ctn}&login={login_mobile}",
		"Resource=1",
	...
	checkResult("api_1_0_info_targetedOffersHistory", lr_eval_string("{response}"), lr_eval_string("{token}"), lr_eval_string("{ctn}"), "");
