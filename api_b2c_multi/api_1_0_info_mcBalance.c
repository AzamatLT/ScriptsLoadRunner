api_1_0_info_mcBalance()
{
	if (pool_get_hash(lr_eval_string("{login}")) == 0) {
		...
		lr_start_transaction("api_1_0_info_mcBalance");
		web_url("status",
				"URL=http://{endpoint}/api/1.0/info/mcBalance?ctn={login}&hash={hash_qouted}",
				"Resource=0",
		...
		lr_end_transaction("api_1_0_info_mcBalance", LR_AUTO);
	}
	return 0;
}
