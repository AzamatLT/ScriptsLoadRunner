api_1_0_info_accumulators()
{
	if (pool_get_hash(lr_eval_string("{login}")) == 0) {
		...
		lr_start_transaction("api_1_0_info_accumulators");
		web_url("accumulators",
				"URL=http://{endpoint}/api/1.0/info/accumulators?ctn={login}&hash={hash_qouted}",
				"Resource=0",
		...
		lr_end_transaction("api_1_0_info_accumulators",LR_AUTO);
	}
	return 0;
}
//&accCode={accCode}
