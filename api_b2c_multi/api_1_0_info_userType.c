api_1_0_info_userType()
{
	if (pool_get_hash(lr_eval_string("{login}")) == 0) {
		...
		lr_start_transaction("api_1_0_info_userType");

		web_url("auth",
				"URL=http://{endpoint}/api/1.0/info/userType?login={login}&hash={hash_qouted}",
				"Resource=1",
		...
		lr_end_transaction("api_1_0_info_userType", LR_AUTO);
	}
	return 0;
}
